...
public class GoogleTranslateTest {
  ...
  private Runnable runMockEnv(AppFixture fixture) {
    return () -> {
      ...
      fixture.config("RestClients.googleTranslate.Features", List.of(CsrfHeaderFeature.class.getName()));
    };
  }

  /**
   * Test case: translate "Hello World" from English to German Expect: the
   * translated is "Hallo Welt"
   */
  @TestTemplate
  ...
  /**
   * Test case: translate "Hello World" from English to English Expect: the
   * translated is "Hello World"
   */
  @TestTemplate
  ...
  /**
   * Test case: translate a text from a blank to English Expect: the translated is
   * "Hallo Welt"(default as German)
   */
  @TestTemplate
  ...
  /**
   * Test case: get a list languages name by English Expect: the list has more 100
   * languages by English
   */
  @TestTemplate
  public void languages_listOfEn(ExtensionContext context, BpmClient bpmClient) {
    ExecutionResult result = bpmClient.start().subProcess(Start.GOOGLE_LANGUAGES).withParam("targetLanguage", "en")
        .execute();
    Data data = result.data().last();
  ...
  /**
   * Test case: get a list languages name by a blank value Expect: the list has
   * more 100 languages by default as English
   */
  @TestTemplate
  public void languages2_listOfBlank(ExtensionContext context, BpmClient bpmClient) {
    ExecutionResult result = bpmClient.start().subProcess(Start.GOOGLE_LANGUAGES).withParam("targetLanguage", "")
        .execute();
    Data data = result.data().last();
    var languages = data.getLanguages();
    assertTrue(languages.size() > 100);
